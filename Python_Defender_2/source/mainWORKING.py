            ...
            for weapons in playerObj.weaponList[:]:
                ...
                if playerObj.isFiring:
                    if weapons.currentReload >= weapons.fireRate and weapons.reactorUse <= playerObj.currentCapacitor:
                        ...
                        if weapons.weaponType == 'laser':
                            playerObj.bulletList.append(bullets.Laser(playerObj, weapons, 3, 'up'))
                        if weapons.weaponType == 'missile':# and len(computerList) > 0:
                            if weapons.missileType == 'light':
                                playerObj.missileList.append(bullets.LightMissile(computers, weapons, 5, 'up'))
                            if weapons.missileType == 'dummy':
                                playerObj.missileList.append(bullets.DummyMissile(computers, weapons, 5, 'up'))
